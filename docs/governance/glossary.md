...
| **Defense-in-Depth Security** | System-wide security strategy where both Frontend (UX pre-validation) and Backend (mandatory 7-stage security guard pipe) participate in validation, while the backend remains the sole authoritative security boundary. | BR-19.3, FR-138, NFR-28 |
...
| **Direct-to-R2 Presigned Upload** | Architecture requesting 15-minute presigned S3/R2 PutObject URLs from NestJS backend (`POST /api/v1/documents/presigned-upload-url`) to transfer files directly from browser to Cloudflare R2, eliminating API server RAM & proxy bandwidth costs. | BR-18.3, FR-134, NFR-27, docs/file-storage-and-upload-policy.md |
| **Dual-Layer Validation Architecture** | Architectural pattern enforcing first-level input pre-validation on client PWA forms for immediate UX feedback and mandatory independent re-validation on NestJS backend for every API request. | BR-19.1–19.5, FR-137–142, NFR-28 |
...
| **Frontend UX Validation** | First-level client-side form pre-checking (required fields, format regex, password rules, 5 MB file size limit) executing before API request dispatch to improve responsiveness and reduce unnecessary network traffic. | BR-19.1, FR-137 |
...
| **Max File Size Exceeded Error** | Standardized backend error envelope (`HTTP 413 PAYLOAD_TOO_LARGE` / `HTTP 400 BAD_REQUEST`, code `MAX_FILE_SIZE_EXCEEDED`) returned when an upload payload exceeds 5 MB. | BR-18.2, FR-132, docs/file-storage-and-upload-policy.md |
...
| **Zero-Trust Client Principle** | Backend security invariant assuming all client input is untrusted and tampered until independently verified by NestJS guards and Zod validation pipes, protecting against direct API bypasses. | BR-19.2, FR-139, NFR-28 |
